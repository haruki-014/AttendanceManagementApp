...
//	            ")";
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    att.setCheckInTime(rs.getTimestamp("check_in_time") != null ? rs.getTimestamp("check_in_time").toLocalDateTime() : null);
                    att.setCheckOutTime(rs.getTimestamp("check_out_time") != null ? rs.getTimestamp("check_out_time").toLocalDateTime() : null);
                    records.add(att);
                }
            }
        } catch (SQLException e) {
    ...
    public List<Attendance> findFilteredRecords(String userId, LocalDate startDate, LocalDate endDate) {
        return attendanceRecords.stream()
                ...
                .filter(att -> endDate == null || (att.getCheckOutTime() != null && !att.getCheckOutTime().toLocalDate().isAfter(endDate)))
